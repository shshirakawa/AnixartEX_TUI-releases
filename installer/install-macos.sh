#!/usr/bin/env bash
# AnixartEX TUI — установщик для macOS (Intel и Apple Silicon)
set -euo pipefail
...
check_platform() {
  if [[ "$(uname -s)" != "Darwin" ]]; then
    die "этот скрипт только для macOS. Для Linux: install-linux.sh"
  fi
}
...
check_node() {
  ...
  if [[ "$major" -lt 24 ]]; then
    die "нужен Node.js 24 LTS+, сейчас: $(node -v). Установи: https://nodejs.org или brew install node"
  fi
}
...
parse_args() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      -h|--help)
        printf '%s\n' \
          "AnixartEX TUI — установщик для macOS" \
          "" \
          "Использование: install-macos.sh [--version VER] [--tag TAG] [--dir PATH]" \
          "" \
          ...
          "" \
          "Требования: Node.js 24 LTS+, Terminal.app / iTerm2 / Warp"
        exit 0
        ;;
      *)
...
download_archive() {
  ...
  if need_cmd curl; then
    ...
    die "нужен curl (встроен в macOS)"
  fi
}
...
install_release() {
  ...
  if [[ ":$PATH:" != *":$BIN_DIR:"* ]]; then
    ...
    log "для zsh (по умолчанию): echo 'export PATH=\"\$HOME/.local/bin:\$PATH\"' >> ~/.zprofile"
  fi
